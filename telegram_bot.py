...
def format_for_telegram(text):
    """Приводит HTML-пост к формату, поддерживаемому Telegram."""
    if not text:
        return ""
    # Заменяем <p>...</p> на двойной перенос строки
    text = re.sub(r'</?p>', '\n\n', text)
    # Заменяем <ul> и </ul> на пусто
    text = re.sub(r'</?ul>', '', text)
    # Заменяем <li>...</li> на • ...\n
    text = re.sub(r'<li>(.*?)</li>', r'• \1\n', text)
    # Удаляем все НЕподдерживаемые теги (например, <h1>, <h2>, <br>, etc.)
    text = re.sub(r'</?(h[1-6]|br|div|span|ol|tr|td|th|table|thead|tbody|tfoot)[^>]*>', '', text)
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": (
                            "Ты — креативный копирайтер для Telegram-канала. "
                            "Сгенерируй структурированный, красивый пост (500–700 символов) с эмодзи, "
                            "разделяй абзацы, используй списки, жирный шрифт для заголовков (используй эмодзи(побольше) или тире для списка), "
                            "а в конце добавь хештеги отдельной строкой. "
                            "Используй только теги <b>, <i>, <u>, <s>, <a>, <code>, <pre>. "
                            "Не используй <p>, <ul>, <li>, <h1> и другие теги."
                        ),
                    },
